...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private final UnitOMRepository unitOMRepository;

    public RecipeBootstrap(CategoryRepository categoryRepository, RecipeRepository recipeRepository, UnitOMRepository unitOMRepository) {
    ...
    @Override
    //@Transactional ...if we see a lazy initialisation Exception
    ...
    private List<Recipe> getRecipes() {
        ...
        //get UOMs
        Optional<UnitOM> eachUomOptional = unitOMRepository.findByDescription("Each");

        //This section is for testing purposes ... should be done as tests with bootstrap data sets.

        if(eachUomOptional.isEmpty()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOM> tableSpoonUomOptional = unitOMRepository.findByDescription("Tablespoon");

        if(tableSpoonUomOptional.isEmpty()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOM> teaSpoonUomOptional = unitOMRepository.findByDescription("Teaspoon");

        if(teaSpoonUomOptional.isEmpty()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOM> dashUomOptional = unitOMRepository.findByDescription("Dash");

        if(dashUomOptional.isEmpty()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOM> pintUomOptional = unitOMRepository.findByDescription("Pint");

        if(pintUomOptional.isEmpty()){
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOM> cupsUomOptional = unitOMRepository.findByDescription("Cup");
        ...
        //get optionals
        UnitOM eachUom = eachUomOptional.get();
        UnitOM tableSpoonUom = tableSpoonUomOptional.get();
        UnitOM teaSpoonUom = teaSpoonUomOptional.get();
        UnitOM dashUom = dashUomOptional.get();
        UnitOM pintUom = pintUomOptional.get();
        UnitOM cupsUom = cupsUomOptional.get();
        ...
        guacamoleRecipe.addIngredient(new Ingredient("serrano chilies, stems and seeds removed, minced", new BigDecimal(2), eachUom, guacamoleRecipe));
